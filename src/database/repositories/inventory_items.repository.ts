...
import { canConvert, convertUnit } from '@/core/utils/unitConversion';
...
export function toDomain(row: InventoryItemRow): InventoryItem {
  return {
    ...
    vatType:        (row.vat_type ?? 'vatable') as 'vatable' | 'vat_exempt' | 'zero_rated',
    isVatInclusive: ((row.is_vat_inclusive ?? 0) as number) === 1,
    vatRate:        (row.vat_rate ?? 0.12) as number,
